...
func TestGenerateFromEmptySnap(t *testing.T) {
	...
	for i := 0; i < 400; i++ {
		...
		helper.addTrieAccount(fmt.Sprintf("acc-%d", i), &types.StateAccount{MntBalances: types.NewQKCTokenBalances(uint256.NewInt(1)), Root: stRoot, CodeHash: types.EmptyCodeHash.Bytes()})
	}
