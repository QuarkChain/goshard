...
func TestBalanceChangeTracksQKCPresence(t *testing.T) {
	...
	// (state.py:166) rather than deleting the key, so the account it reconstructs
	// holds zero rather than holding nothing. Restoring the marker here would
	// serialize as empty bytes and hash to a different leaf.
