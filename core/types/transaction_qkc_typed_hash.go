...
func receiptToHex(data *common.Address) string {
	if data == nil {
		return "0x"
	}
...
func qkcTxToTypedData(qkcTx *QkcTx) []map[string]string {
	...
	typedTxData = append(typedTxData, map[string]string{
		...
		"value": uint32ToHex(qkcTx.fromFullShardKey()),
	})
	typedTxData = append(typedTxData, map[string]string{
		"type":  "uint32",
		"name":  "toFullShardKey",
		"value": uint32ToHex(qkcTx.toFullShardKey()),
	})
	typedTxData = append(typedTxData, map[string]string{
		"type":  "uint64",
		"name":  "gasTokenId",
...
func solidityPack(types, values []string) ([]byte, error) {
	...
	for index, t := range types {
		...
		if t == "bytes" {
			...
			retv = append(retv, strRJust(v, byte(0), size)...)
			...
			retv = append(retv, strRJust(v, byte(0), int(size)/8)...)
		} else {
...
func soliditySha3(types, value []string) (string, error) {
	...
	return crypto.Keccak256Hash(packData).String(), nil
}
