// Ported from github.com/QuarkChain/goquarkchain/consensus (faithful).
// Adaptations:
//   - hashicorp/golang-lru -> geth common/lru (typed; not in this module's deps).
//   - CreatePoSWCalculator is omitted: PoSW lives in the deferred execution issue.
...
type CommonEngine struct {
	...
	sealVerifiedCache *lru.Cache[common.Hash, *big.Int]
}
...
func (c *CommonEngine) mine(
	...
	var (
		// pyquarkchain boundary: floor(2^256/diff)-1, strict "<" (matches double256 verifySeal)
		target   = new(big.Int).Sub(new(big.Int).Div(two256, work.Difficulty), big.NewInt(1))
		minerRes = ShareCache{
			Height:    work.Number,
...
func NewCommonEngine(spec MiningSpec, diffCalc DifficultyCalculator, remote bool, pubKey []byte) *CommonEngine {
	cache := lru.NewCache[common.Hash, *big.Int](512)
	c := &CommonEngine{
		spec:              spec,
		diffCalc:          diffCalc,
		pubKey:            pubKey,
...
// TODO(execution-issue): CreatePoSWCalculator(cr ChainReader, poswConfig *config.POSWConfig)
// lived here in goquarkchain; PoSW depends on staked balances from the state layer
// and is part of the deferred execution issue.
