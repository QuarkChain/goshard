// Ported from github.com/QuarkChain/goquarkchain/consensus/doublesha256 (faithful).
...
func verifySeal(chain consensus.ChainReader, header types.IHeader, adjustedDiff *big.Int) error {
	...
	// Follow pyquarkchain (cluster/miner.py): the double-SHA256 PoW boundary is
	// floor(2^256/diff) - 1 with a strict "<". goquarkchain uses floor(2^256/diff)
	// with "<=", which accepts two boundary results a pyquarkchain node rejects
	// (a POW_DOUBLESHA256 consensus mismatch).
	target := new(big.Int).Sub(new(big.Int).Div(two256, diff), big.NewInt(1))
