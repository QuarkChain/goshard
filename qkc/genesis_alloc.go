...
func commitGenesisAlloc(db ethdb.Database, alloc map[account.Address]config.Allocation) (common.Hash, error) {
	...
	for addr, allocation := range alloc {
		...
		if acct.Nonce == 0 && acct.MntBalances.IsBlank() && bytes.Equal(acct.CodeHash, coretypes.EmptyCodeHash.Bytes()) {
			continue
		}
		if storageNodes != nil {
	...
	// decoding them as types.StateAccount (triedb/hashdb/database.go:572-583), which is
	// the QuarkChain leaf here, so the storage tries become reachable from the
...
func genesisAccount(tdb *triedb.Database, batch ethdb.Batch, addr account.Address, allocation config.Allocation) (*coretypes.StateAccount, *trienode.NodeSet, error) {
	acct := coretypes.NewEmptyStateAccount()
	acct.FullShardKey = addr.FullShardKey
