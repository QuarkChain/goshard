...
// Cross-shard transaction tests exercise versioned QKC wire bytes.
...
func TestCrossShardTransactionListGolden(t *testing.T) {
	...
	assertSerialized(t, NewCrossShardTransactionList(nil), "000000000000000400000000")
	...
	assertSerialized(t, NewCrossShardTransactionList([]*CrossShardTransactionDeposit{{
		...
		RefundRate:      10,
	}}), "")
}

func TestCrossShardTransactionListRejectsUnsupportedVersion(t *testing.T) {
	var list CrossShardTransactionList
	err := serialize.DeserializeFromBytes(common.FromHex("000000010000000400000000"), &list)
	assert.Error(t, err)
}

func TestCrossShardTransactionListPreservesFollowingFields(t *testing.T) {
	type container struct {
		List *CrossShardTransactionList
		Tail uint32
	}

	original := container{List: NewCrossShardTransactionList(nil), Tail: 42}
	encoded, err := serialize.SerializeToBytes(original)
	assert.NoError(t, err)

	var decoded container
	err = serialize.DeserializeFromBytes(encoded, &decoded)
	assert.NoError(t, err)
	assert.Empty(t, decoded.List.TXList)
