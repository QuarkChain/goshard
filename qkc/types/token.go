...
func (t *TokenBalances) Serialize(w *[]byte) error {
	...
	// omit zero-balance entries from both the count and the body. goquarkchain emits
	// them, which also makes Serialize/Deserialize asymmetric (Deserialize drops
	// zeros), so skipping keeps the round-trip and the pyquarkchain bytes consistent.
