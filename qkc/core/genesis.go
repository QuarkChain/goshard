// Ported from github.com/QuarkChain/goquarkchain/core (faithful signatures & flow).
// Modified from go-ethereum under GNU Lesser General Public License
//
// TODO(execution-issue): CreateMinorBlock populated the genesis state trie from
// genesis.Alloc via the forked goquarkchain state (multi-token balances, code,
// storage, full-shard keys) and derived Meta.Root from it. Until the execution
// layer lands, the genesis block carries the empty trie root and the allocation
// is not materialised.
package core
...
func (g *Genesis) CreateMinorBlock(rootBlock *types.RootBlock, fullShardId uint32, db ethdb.Database) (*types.MinorBlock, error) {
	...
	// TODO(execution-issue): build the genesis state from genesis.Alloc here
	// (AddBalance per token, SetCode, SetNonce, SetState, SetFullShardKey) and
	// set Root to the committed state root. The structural genesis uses the
	// empty trie root.
	meta := types.MinorBlockMeta{
		Root:              types.EmptyTrieHash,
	...
	coinbaseAmountTokens := types.NewEmptyTokenBalances()
	coinbaseAmount := qkcCommon.BigIntMulBigRat(shardConfig.CoinbaseAmount, g.qkcConfig.LocalFeeRate)
	coinbaseAmountTokens.SetValue(coinbaseAmount, g.qkcConfig.GetDefaultChainTokenID())
...
// SetupGenesisRootBlock writes or updates the genesis block in db.
...
// MustCommitRootBlock writes the genesis block and state to db, panicking on error.
// The block is committed as the canonical head block.
...
// MustCommitMinorBlock writes the genesis block and state to db, panicking on error.
// The block is committed as the canonical head block.
