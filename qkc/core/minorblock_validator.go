// Ported from github.com/QuarkChain/goquarkchain/core (faithful structural checks).
// Modified from go-ethereum under GNU Lesser General Public License
//
// TODO(execution-issue): the state-dependent parts of ValidateState (gas used,
// xshard cursor, coinbase amount, state root — which need the executed StateDB)
// are seams for the deferred execution issue; the receipt-derived checks are kept.
...
func (v *MinorBlockValidator) ValidateBlock(mBlock types.IBlock, force bool) error {
	...
	// Follow pyquarkchain (cluster/shard_state.py validate_block): tracking_data is
	// size-limited the same as extra_data. goquarkchain omits this check.
	if len(block.TrackingData()) > int(v.quarkChainConfig.BlockExtraDataSizeLimit) {
		log.Error(v.logInfo, "err", ErrExtraLimit, "len block's tracking data", len(block.TrackingData()), "config's limit", v.quarkChainConfig.BlockExtraDataSizeLimit)
		return ErrExtraLimit
	}
...
// ValidateSeal validate minor block seal when validate block
...
func (v *MinorBlockValidator) ValidateState(mBlock, parent types.IBlock, statedb *state.StateDB, receipts types.Receipts, usedGas uint64) error {
	...
	// TODO(execution-issue): the remaining checks of goquarkchain's ValidateState
	// require the executed StateDB (GetGasUsed, GetTxCursorInfo, GetBlockFee,
	// GetXShardReceiveGasUsed, IntermediateRoot) and the multi-token coinbase
	// amount; they are part of the deferred execution issue:
	//   - gas used vs statedb.GetGasUsed()
	//   - xshard tx cursor vs statedb.GetTxCursorInfo()
	//   - coinbase amount vs getCoinbaseAmount + statedb.GetBlockFee()
	//   - xshard receive gas used vs statedb.GetXShardReceiveGasUsed()
	//   - state root vs statedb.IntermediateRoot(true)
	return nil
}
