...
type executionContext struct {
	...
	// Where the current message's logs start, see BeginMessage. state.logs is
	// in STATE_DEFAULTS (state.py:45), so the watermark naming them has to
	// travel with the snapshot: reverting past a BeginMessage must put back the
	// message the logs are attributed to, not just unwind the logs themselves.
...
// (SetNonce, SetCode), and Snapshot/RevertToSnapshot/Commit have to cover the
// execution context as well as the accounts. Reach the geth spelling through
// the embedded field when one is genuinely wanted.
...
func (s *EvmState) BeginMessage(txHash common.Hash, index int) {
	...
	s.ctx.messageHash = txHash
	s.ctx.messageIndex = index
	s.ctx.messageLogsFrom = len(s.StateDB.GetLogs(txHash, 0, common.Hash{}, 0))
}

// MessageLogs returns the logs emitted since BeginMessage.
func (s *EvmState) MessageLogs() []*coretypes.Log {
	all := s.StateDB.GetLogs(s.ctx.messageHash, s.ctx.blockNumber, common.Hash{}, s.ctx.timestamp)
	if s.ctx.messageLogsFrom >= len(all) {
		return nil
	}
	return all[s.ctx.messageLogsFrom:]
}

// Receipts and XShardDepositReceipts are kept apart because the block's receipt
// root is the two lists concatenated, in that order (shard_state.py:957). Both
// are per-transaction state the caller clears between transactions, as
// apply_transaction does (messages.py:419-421).
...
func (s *EvmState) RevertToSnapshot(id int) {
	...
	s.ctx = rev.ctx
	// Two pieces of the context are mirrored on the StateDB and have to be
	// pushed back to it: the shard key stamped on accounts as they are read,
	// and the message the logs written from here on are attributed to.
	s.StateDB.SetFullShardKey(s.ctx.fullShardKey)
	s.StateDB.SetTxContext(s.ctx.messageHash, s.ctx.messageIndex)
...
func (s *EvmState) Commit(block uint64) (common.Hash, error) {
	...
	// the same guarantee without depending on how much of geth's caching survives
	// a commit — including that a balance entry left at zero is gone once the leaf
	// has been through the trie, since the pair list carries no zeros.
